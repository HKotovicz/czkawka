...
rust_optimize_image_confirmation = Are you sure you want to optimize the selected images?
rust_optimize_image_confirmation_number_simple = The operation will be performed on { $items } items.
rust_optimize_image_summary = Optimized {$optimized} images (skipped {$failed} of {$total})
rust_no_images_optimized = No images optimized
rust_found_image_files = Found {$items_found} image files in {$time}.
optimize_images_text = Optimize Images
optimize_image_confirmation_text = Are you sure you want to convert/compress the selected images?
optimize_image_target_format_text = Target Format
optimize_image_quality_text = Quality
preserve_metadata_text = Preserve metadata
...
main_window_title = Krokiet (Croquette) - Data Cleaner
...
motto = This program is free to use and will always be. See the MIT/GPL License for details.
...
build_info_build = Build
build_info_runtime = Runtime
build_info_refresh = Refresh
build_info_diagnostic_button = Diagnostic Info
build_info_diagnostic_title = Diagnostic Info
build_info_library_restart_hint = * Library probes (HEIF, LibRAW, AVIF) reflect the state at startup - restart the app after installing or removing plugins.
build_info_test_file = Test File
other_apps_title = Other applications by qarmin
other_apps_open_source_note = All listed applications are free and open source.
other_apps_open_button = Open
other_apps_szyszka_desc = Fast and powerful file renamer.
other_apps_mykrut_desc = Simple fast and opinionated Linux file manager.
other_apps_dcmki_viewer_desc = Simple DICOM viewer.
other_apps_video_thumbnailer_desc = Wrapper around the video thumbnail generator used in Czkawka.
other_apps_space_finder_desc = Simple finder of the biggest files on your system.
other_apps_system_info_collector_desc = Collects RAM/CPU usage from the OS and shows it as graphs.
...
tool_image_optimizer = Image Optimizer
...
selection_by_file_type = Select by file type
popup_select_by_file_type_title_text = Select by File Type
popup_select_by_file_type_header_text = File type
popup_select_by_file_type_select_text = Select
popup_select_by_file_type_unselect_text = Unselect
...
subsettings_video_optimizer_hardware_decoder = Hardware Decoder
subsettings_video_optimizer_hardware_decoder_hint = "Auto" uses the same GPU as the encoder. Set to a different GPU to decode on one device and encode on another (e.g. CUDA decode + AMF AV1 encode).
...
context_menu_rename_text = Rename
